...
const LLAMA_CPP_BASE_URL = "http://127.0.0.1:8765/v1";

/**
 * Build the llama-cpp-python provider config by probing the local server.
 * The server is launched via `scripts/llama_cpp_server.py` which starts
 * llama-cpp-python's built-in OpenAI-compatible HTTP server.
 *
 * Models are marked as supporting image input so that vision-capable GGUF
 * models (LLaVA, Moondream) work with OpenClaw's browser screenshot features.
 */
export async function buildLlamaCppProvider(params?: {
  baseUrl?: string;
  apiKey?: string;
}): Promise<ProviderConfig> {
  const baseUrl = (params?.baseUrl?.trim() || LLAMA_CPP_BASE_URL).replace(/\/+$/, "");
  const discoveredModels = await discoverOpenAICompatibleLocalModels({
    baseUrl,
    apiKey: params?.apiKey,
    label: "llama-cpp-python",
  });
  // Annotate all discovered models with vision input support so that
  // vision-capable GGUF models (LLaVA, Moondream) are usable out-of-the-box.
