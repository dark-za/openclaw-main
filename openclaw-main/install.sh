...
# ── Post-install: fix permissions & generate gateway auth ─────────────────────
step "🔐 Configuring OpenClaw runtime"

OPENCLAW_STATE="$HOME/.openclaw"
OPENCLAW_ENV="$OPENCLAW_STATE/.env"
mkdir -p "$OPENCLAW_STATE"

# Fix group-writable state directory (security warning from openclaw status)
chmod 700 "$OPENCLAW_STATE" 2>/dev/null && ok "State dir secured (chmod 700 ~/.openclaw)" || true

# Generate gateway auth token if not already set
if [ -f "$OPENCLAW_ENV" ] && grep -q "OPENCLAW_GATEWAY_TOKEN" "$OPENCLAW_ENV" 2>/dev/null && \
   ! grep -q "change-me" "$OPENCLAW_ENV" 2>/dev/null; then
  ...
  if command -v openssl &>/dev/null; then
    ...
    TOKEN="$(cat /dev/urandom | tr -dc 'a-f0-9' | head -c 64 2>/dev/null || echo "")"
  fi

  if [ -n "$TOKEN" ]; then
    # Preserve existing .env content if any, then add/replace token line
    if [ -f "$OPENCLAW_ENV" ]; then
      # Remove old token line if present
      grep -v "OPENCLAW_GATEWAY_TOKEN" "$OPENCLAW_ENV" > "${OPENCLAW_ENV}.tmp" 2>/dev/null && \
        mv "${OPENCLAW_ENV}.tmp" "$OPENCLAW_ENV" || true
    fi
    {
      echo "# Auto-generated by install.sh on $(date '+%Y-%m-%d')"
      echo "# OpenClaw gateway auth — keep this secret"
      echo "OPENCLAW_GATEWAY_TOKEN=$TOKEN"
      echo ""
      echo "# Uncomment to use a model provider API key:"
      echo "# ANTHROPIC_API_KEY=sk-ant-..."
      echo "# OPENAI_API_KEY=sk-..."
      echo ""
      echo "# HuggingFace token (speeds up model downloads):"
      echo "# HF_TOKEN=hf_..."
    } >> "$OPENCLAW_ENV"
    chmod 600 "$OPENCLAW_ENV"
    ok "Gateway auth token generated → ~/.openclaw/.env"
    ok "File permissions secured (chmod 600)"
  else
    warn "Could not generate token — run manually: openssl rand -hex 32"
  fi
fi
...
echo -e "  ${BOLD}Auth     ${RESET}: gateway token → ~/.openclaw/.env ${GREEN}✓${RESET}"
echo ""
echo -e "  ${BOLD}${CYAN}▶  Start everything with one command:${RESET}"
echo -e "    ${CYAN}cd $PROJECT_ROOT && bash bin/openclaw-start.sh${RESET}"
echo ""
echo -e "  ${BOLD}Or step by step:${RESET}"
echo -e "    1. bash bin/openclaw-llm.sh     ${GREEN}# inference server${RESET}"
echo -e "    2. pnpm start                   ${GREEN}# OpenClaw gateway${RESET}"
echo ""
echo -e "  ${BOLD}Dashboard:${RESET}  http://127.0.0.1:18789/"
echo -e "  ${BOLD}API docs:${RESET}   https://docs.openclaw.ai"
sep
